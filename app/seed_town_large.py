"""美味小镇食材大全生成器（v0.1.7 · spec 233 食材体系）

按 spec《美味小镇物品资料总览》补齐 233 食材（v0.1.6 前仅 14 种）。
spec 食材分级：一级22 / 二级65 / 三级50 / 四级30 / 五级30 / 六级神秘10 / 六级其他10 / 万能5 = 222
本生成器按 spec 列出的具名食材落地，不足部分按命名规律补全至 233。

设计：
- 每级食材按 spec 名称列表落地，售价随级别递增（1级5/2级10/3级20/4级40/5级80/6级150）
- 万能食材 1-5 级（v0.1.1 已有 6 级万能食材，此处补 1-5 级）
- 同步落 Item 字典 module_key=town

幂等：以 key 存在性判断，已存在则跳过；可断点续跑。
闭环：全部食材落 Item 字典，可被菜谱 ingredients JSON 引用。
"""
...
async def seed_town_large(db: AsyncSession, log=print):
    """幂等生成美味小镇 233 食材；返回新增计数"""
    stats = {"items": 0}
    ...
    pinyin_map = {
        # 一级
        "醋": "cu", "白菜": "baicai", "猪肉": "zhurou", "黄瓜": "huanggua", "紫苏": "zisu",
        "鸡蛋": "jidan", "番茄": "fanqie", "茄子": "qiezi", "糯米": "nuomi", "白糖": "baitang",
        "大蒜": "dasuan", "辣椒": "lajiao", "大葱": "dacong", "香葱": "xiangcong", "生姜": "shengjiang",
        "面粉": "mianfen", "薏仁": "yiren", "糙米": "caomi", "土豆": "tudou", "生菜": "shengcai",
        "扁豆": "biandou", "干辣椒": "ganlajiao",
    }

    async def _add_ing(name: str, level: int, idx: int, tag: str = ""):
        """落 Item 字典食材，key = town_ing_lv{level}_{idx}"""
        ...
        if not await goods.get_item_by_key(db, key):
            ...
            await goods.ensure_item(db, key, name, "ingredient", "town", True, price, desc)
            stats["items"] += 1

    # 落各级食材
    for i, name in enumerate(lv1, 1):
        await _add_ing(name, 1, i)
    for i, name in enumerate(lv2, 1):
        await _add_ing(name, 2, i)
    for i, name in enumerate(lv3, 1):
        await _add_ing(name, 3, i)
    for i, name in enumerate(lv4, 1):
        await _add_ing(name, 4, i)
    for i, name in enumerate(lv5, 1):
        await _add_ing(name, 5, i)
    for i, name in enumerate(lv6_mystery, 1):
        await _add_ing(name, 6, i, "神秘")
    for i, name in enumerate(lv6_other, 1):
        await _add_ing(name, 6, i + 100, "西式")

    # 万能食材 1-5 级（v0.1.1 已有 6 级 town_wild_ing_1~6，此处补 1-5 级确保完整）
    for lvl in range(1, 6):
        key = f"town_wild_ing_{lvl}"
        if not await goods.get_item_by_key(db, key):
            price = [30, 50, 80, 120, 180][lvl - 1]
            await goods.ensure_item(db, key, f"{lvl}级万能食材", "ingredient", "town", True,
                                    price, f"合菜替代位·{lvl}级")
            stats["items"] += 1
