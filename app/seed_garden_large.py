"""魔法花园超大数据生成器（v0.1.4 · spec 大全级资料库）

按 spec《物品体系分册》《订单系统分册》生成可无限扩展的资料库样例规模：
...
闭环：
  - 作物 item_key 进入订单模板需求池与配方材料
  - 配方产出 seed（target tier 2-8），消耗低阶 material/bloom
  - 订单模板按 level 分层，需求 item 来自 bloom/material 字典
价值：value_coin = max(sell,1)*4（与 garden.item_value_coin 的 grow_seconds=0 分支一致），
      卖价/解锁/成长时长按 tier 分档，符合 spec 四段式时间价值 + 稀有溢价。
"""
import json
import random
...
# ---------- tier 配置（spec：等级门槛 + 任务门槛双条件，此处用等级门槛）----------
...
def _value_coin(sell_price: int) -> int:
    """spec：非成长物 value_coin = max(sell,1)*4（对齐 item_value_coin grow_seconds=0 分支）"""
...
async def seed_garden_large(db: AsyncSession, log=print):
    """幂等生成超大数据；返回各分类新增计数"""
    stats = {"crops": 0, "materials": 0, "recipes": 0, "templates": 0}

    # ---------- 1. 作物 520（bloom + seed + album + item）----------
    bloom_cnt = await _count(db, select(models.GardenBloom).where(
        models.GardenBloom.key.like("gbloom_t%")))
    if bloom_cnt < CROP_PER_TIER * len(TIERS):
        # 已存在的 key 集合（避免重复）
        ex_bloom = {r[0] for r in (await db.execute(
            select(models.GardenBloom.key).where(models.GardenBloom.key.like("gbloom_t%")))).all()}
        ex_seed = {r[0] for r in (await db.execute(
            select(models.GardenSeed.key).where(models.GardenSeed.key.like("gseed_t%")))).all()}
        ex_album = {r[0] for r in (await db.execute(
            select(models.GardenAlbumEntry.key).where(models.GardenAlbumEntry.key.like("galbum_t%")))).all()}
        ex_item = {r[0] for r in (await db.execute(
            select(models.Item.key).where(models.Item.key.like("garden_bloom_t%")))).all()}
        ex_seed_item = {r[0] for r in (await db.execute(
            select(models.Item.key).where(models.Item.key.like("garden_seed_t%")))).all()}
        ...
        await _bulk(db, add_items)
        await _bulk(db, add_albums)
        await _bulk(db, add_blooms)
        await _bulk(db, add_seeds)
        stats["crops"] = len(add_blooms)
        log(f"[garden-large] 作物 +{len(add_blooms)} (bloom/seed/album/item 同步)")

    # ---------- 2. 材料 1024（item 字典）----------
    mat_cnt = await _count(db, select(models.Item).where(
        models.Item.key.like("garden_mat_t%")))
    if mat_cnt < MAT_PER_TIER * len(TIERS):
        ex_mat = {r[0] for r in (await db.execute(
            select(models.Item.key).where(models.Item.key.like("garden_mat_t%")))).all()}
        add_items = []
        for tier in TIERS:
            ...
            for i in range(MAT_PER_TIER):
                ...
                if key in ex_mat:
                    continue
                add_items.append(models.Item(key=key, name=f"{base}{suf}·T{tier}",
    ...
    bloom_rows = (await db.execute(select(models.GardenBloom).where(
        models.GardenBloom.key.like("gbloom_t%")))).scalars().all()
    mat_rows = (await db.execute(select(models.Item).where(
        models.Item.key.like("garden_mat_t%")))).scalars().all()
    seed_rows = (await db.execute(select(models.GardenSeed).where(
        models.GardenSeed.key.like("gseed_t%")))).scalars().all()
    # 按 tier 分桶
    ...
    recipe_cnt = await _count(db, select(models.GardenRecipe).where(
        models.GardenRecipe.name.like("配方#R%")))
    ...
    if need_recipes > 0 and seed_rows:
        ...
        while made < need_recipes:
            ...
            if not mats:
                ...
                continue
            sr = max(30, 92 - (tt - 1) * 8)
            add_recipes.append(models.GardenRecipe(
    ...
    if need_tpls > 0 and (bloom_rows or mat_rows):
        ...
        while made < need_tpls:
            ...
            for _ in range(n_items):
                # spec 闭环：订单以收获产出(bloom)为主，材料为辅(70/30)
