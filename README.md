...
以用户提供的《QQ家园魔法花园完整游戏资料》+ WAP 原版首页布局为权威依据，重新设计魔法花园首页：在**不破坏现有花盆/合成/花谱/订单/任务系统**的前提下，把首页重构为原版 WAP 风貌（顶导航/公告/活动/玩家信息栏/天气/花盆区/消息/花园设施/公告板/底导航），并新增精灵花册·花之图谱·普通/独特/珍稀三项原版收集进度。策略与 v0.2.7 一致（保守增量，参考层并存）。

**一、原版 WAP 布局参考层（garden_data.py +11 组常量，纯静态不入库）**

| 常量 | 内容 | 对应原版布局 |
|------|------|-------------|
| `HOME_NAV_TOP` | 5 项顶导航（花园/好友/花房/魔法屋/活动） | 原版主菜单 |
| `HOME_NAV_BOTTOM` | 8 项底导航（排行/背包/帮助/论坛/种子商店/道具商店/游戏活动/魔钻专区） | 原版底栏 |
| `WEATHER_MESSAGES` | 6 条天气消息池 | 原版"今天阳光明媚…"天气提示 |
| `GARDEN_FACILITIES` | 5 项花园设施 | 原版花园设施区 |
| `BLUE_DIAMOND_TIP` | 蓝钻尊享一键特权提示 | 原版蓝钻横幅 |
| `SPIRIT_BOOK_TOTAL` | 精灵花册总数 8（7 步任务链 + 1 完成态） | 原版"精灵花册:0/8" |
| `RARITY_BUCKET_MAP` / `RARITY_BUCKETS` | 稀有度分桶映射（普通/独特/珍稀） | 原版"普通:0独特:0珍稀:0" |
| `ANNOUNCE` / `BOARD_MSG` | 首页公告 / 公告板文案 | 原版公告区 |

**二、首页路由数据补全（garden.py home 路由 +9 项计算）**

- `weather_msg`：按日期轮换的天气消息（每日固定）
- `spirit_done`/`spirit_total`：精灵花册进度（魔法任务链 quest_step → 完成数/8）
- `atlas_total`：花之图谱总数（花谱条目总数，与点亮数 lit_count 配对）
- `rarity_counts`：普通/独特/珍稀计数（点亮花谱项 → 花朵 rarity 映射：普通→普通，稀有→独特，史诗/传说→珍稀）
- `empty_pots`/`planted_pots`：空花盆/已种植花盆数
- `unread`：未读消息数（查 Message 表）
- `garden_name`：花园名（"{昵称}的花园"）
- 传入 `D=garden_data` 供模板遍历导航/设施/稀有度

**三、首页模板重设计（garden/home.html 全量重写）**

按原版 WAP 布局从上到下重构：
1. 顶部导航 5 项（花园/好友/花房/魔法屋/活动）
2. 公告 + 活动区（魔法任务/活动兑换/可收获/待护理）
3. 玩家信息栏（花园名 + 用户名 + 称号(经验条) + 精灵花册 + 花之图谱 + 普通/独特/珍稀 + 金币/花盆/物品上限）
4. 天气提示
5. 花盆区（花盆(N) 刷新 + 【全部花盆|已种植】 + 蓝钻横幅 + 逐盆状态列表 + 空花盆[播种]）
6. 消息（消息中心 + 未读数）
7. 花园设施（5 设施 tag）
8. 公告板 + 花园名
9. 底部导航 8 项（排行/背包/帮助/论坛/种子商店/道具商店/游戏活动/魔钻专区）

**四、设计原则：布局对齐，系统不动**

- 现行可玩系统（花盆状态机/合成工坊/花谱点亮/订单板/魔法任务链/装饰环境值/好友偷花送花）**完全保留**，未做任何数值改动
- 新增常量为**纯参考层**，仅用于首页布局展示，不参与种植/合成/收获判定
- 稀有度计数通过现有 GardenBloom.rarity 映射，不新增字段；精灵花册复用魔法任务链 quest_step，不新增表

**五、闭环验证**
- ✅ IMPORT OK：`VERSION = 0.2.8`；11 组布局常量全部可访问
- ✅ 数据校验：HOME_NAV_TOP 5 / HOME_NAV_BOTTOM 8 / WEATHER 6 / FACILITIES 5 / RARITY_BUCKETS 3 / SPIRIT_BOOK_TOTAL 8 —— 与原版布局一致
- ✅ 元组 arity：导航项 (name, href) 二元组与模板 `{% for name, href in ... %}` 解包匹配
- ✅ 路由数据：weather/spirit/atlas/rarity_counts/empty_pots/unread/garden_name 全部传入模板上下文
- ✅ 模板：home.html 重写后无未定义变量引用

**文件变更**
- `app/routers/garden_data.py`：+v0.2.8 原版 WAP 布局参考层（11 组常量）
- `app/routers/garden.py`：home 路由 +9 项数据计算 + 传入 D；import garden_data as D + func
- `app/templates/garden/home.html`：全量重写为原版 WAP 布局
- `app/config.py`：版本号 0.2.7 → 0.2.8
